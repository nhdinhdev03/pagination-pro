...
// Generate mock items
const now = Date.now();
export const items: Item[] = Array.from({ length: 250 }).map((_, i) => ({
  id: i + 1,
  name: `Item #${i + 1}`,
  createdAt: new Date(now - i * 60_000).toISOString() // 1 min apart
}));

export async function findManyAndCount(opts: { page: number; limit: number; sort: string }) {
  const { page, limit, sort } = opts;
  const [key, dir] = (sort || "createdAt:desc").split(":");
  const sorted = [...items].sort((a, b) => {
    const av = a[key as keyof Item] as any;
    const bv = b[key as keyof Item] as any;
    if (av === bv) return 0;
    const cmp = av > bv ? 1 : -1;
  ...
  return [slice, items.length] as const;
}
...
export async function findPageByCursor(opts: {
  ...
  const sorted = [...items].sort((a, b) => {
    const key = sortKey;
    if (a[key] === b[key]) return direction === "desc" ? b.id - a.id : a.id - b.id;
